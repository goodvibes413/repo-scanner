...
from repo_preflight.models import SourceStatus, Verdict, render_markdown
...
def run() -> None:
    tests = [
        test_flags_root_postinstall,
        test_flags_workspace_postinstall,
        test_flags_curl_bash_and_bun_install,
        test_flags_suspicious_shai_hulud_filenames,
        test_flags_unexpected_lockfile_tarball_url,
        test_report_distinguishes_boundaries_and_avoids_forbidden_words,
        test_install_context_uses_names_only,
        test_mcp_scan_matches_cli_function,
        test_osv_unavailable_degrades_dependency_claim,
    ]
    for test in tests:
        with tempfile.TemporaryDirectory(prefix=f"{test.__name__}-") as directory:
            test(Path(directory))
...
def test_flags_unexpected_lockfile_tarball_url(tmp_path: Path) -> None:
    write(
        ...
        json.dumps({"packages": {"node_modules/example": {"version": "1.2.3", "resolved": "https://evil.example/example-1.2.3.tgz"}}}),
    )
    report = scan_repository(str(tmp_path), osv_enabled=False)
    assert report.verdict == Verdict.TEST_ONLY_IN_ISOLATION
    ...
    response = handle_request(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "scan_repository", "arguments": {"repo_url": str(tmp_path), "osv_enabled": False}},
        }
    )
    assert response is not None
    assert response["result"]["structuredContent"]["verdict"] == direct.verdict.value


def test_osv_unavailable_degrades_dependency_claim(tmp_path: Path) -> None:
    import repo_preflight.scanner as scanner

    original = scanner.query_osv

    def fake_query_osv(dependencies, enabled=True):
        return [], SourceStatus("osv", "unavailable", "test outage")

    try:
        scanner.query_osv = fake_query_osv
